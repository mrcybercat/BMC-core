...
public abstract class IntegerColorData  extends ColorData<Integer> {
    public IntegerColorData(LinkedHashMap<ColorChannelName, Integer[][]> colorChannels) {
        super(colorChannels);
    }

    public double getChannelAverageAtXY(int x, int y) {
        double sum = 0;
        for (Map.Entry<ColorChannelName, Integer[][]> entry : this.colorChannels.entrySet()) {
