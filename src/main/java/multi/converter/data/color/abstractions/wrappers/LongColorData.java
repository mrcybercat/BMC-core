...
public abstract class LongColorData  extends ColorData<Long> {
    public LongColorData(LinkedHashMap<ColorChannelName, Long[][]> colorChannels) {
        super(colorChannels);
    }

    public double getChannelAverageAtXY(int x, int y) {
        double sum = 0;
        for (Map.Entry<ColorChannelName, Long[][]> entry : this.colorChannels.entrySet()) {
