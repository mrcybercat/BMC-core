...
public class QuantizeBlocksStep extends AlgorithmStep<BlockData, BlockData> {
    ...
    public BlockData performAlgorithmStep(BlockData source) throws UnableToPerformStepException {
        return new BlockData(
                quantizeLuminaceBlocks(source.getBlocksY()),
                quantizeChromaBlocks(source.getBlocksU()),
                quantizeChromaBlocks(source.getBlocksV())
        );
    }

    private DataBlock[] quantizeLuminaceBlocks(DataBlock[] blocks){
    ...
    private DataBlock quantize(DataBlock block, double[][] quantMatrix){
